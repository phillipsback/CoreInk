...
{
  int16_t x = (M5.Ink.width() - 200) / 2;
  int16_t y = (M5.Ink.height() - 200) / 2;
...
{
  int16_t w2 = M5.Ink.width() / 2;
  int16_t h2 = M5.Ink.height() / 2;
...
static const uint16_t max_row_width = 1872; // for up to 7.8" M5.Ink 1872x1404
...
uint16_t rgb_palette_buffer[max_palette_pixels]; // palette buffer for depth <= 8 for buffered graphics, needed for 7-color M5.Ink
...
void showBitmapFrom_HTTP(const char* host, const char* path, const char* filename, int16_t x, int16_t y, bool with_color)
{
  ...
  if ((x >= M5.Ink.epd2.WIDTH) || (y >= M5.Ink.epd2.HEIGHT)) return;
  ...
  if (read16(client) == 0x4D42) // BMP signature
  {
    ...
    if ((planes == 1) && ((format == 0) || (format == 3))) // uncompressed is handled, 565 also
    {
      ...
      uint16_t w = width;
      uint16_t h = height;
      if ((x + w - 1) >= M5.Ink.epd2.WIDTH)  w = M5.Ink.epd2.WIDTH  - x;
      if ((y + h - 1) >= M5.Ink.epd2.HEIGHT) h = M5.Ink.epd2.HEIGHT - y;
      if (w <= max_row_width) // handle with direct drawing
      {
        ...
        for (uint16_t row = 0; row < h; row++, rowPosition += rowSize) // for each line
        {
          ...
          M5.Ink.writeImage(output_row_mono_buffer, output_row_color_buffer, x, yrow, w, 1);
        } // end line
...
void drawBitmapFrom_HTTP_ToBuffer(const char* host, const char* path, const char* filename, int16_t x, int16_t y, bool with_color)
{
  ...
  bool valid = false; // valid format to be handled
  bool flip = true; // bitmap is stored bottom-to-top
  bool has_multicolors = (M5.Ink.epd2.panel == GxEPD2::ACeP565) || (M5.Ink.epd2.panel == GxEPD2::GDEY073D46);
  uint32_t startTime = millis();
  if ((x >= M5.Ink.width()) || (y >= M5.Ink.height())) return;
  ...
  if (read16(client) == 0x4D42) // BMP signature
  {
    ...
    if ((planes == 1) && ((format == 0) || (format == 3))) // uncompressed is handled, 565 also
    {
      ...
      uint16_t w = width;
      uint16_t h = height;
      if ((x + w - 1) >= M5.Ink.width())  w = M5.Ink.width()  - x;
      if ((y + h - 1) >= M5.Ink.height()) h = M5.Ink.height() - y;
      //if (w <= max_row_width) // handle with direct drawing
      {
        valid = true;
        uint8_t bitmask = 0xFF;
        ...
        for (uint16_t row = 0; row < h; row++, rowPosition += rowSize) // for each line
        {
          ...
          for (uint16_t col = 0; col < w; col++) // for each pixel
          {
            ...
            M5.Ink.drawPixel(x + col, yrow, color);
          } // end pixel
        } // end line
      }
      Serial.print("bytes read "); Serial.println(bytes_read);
